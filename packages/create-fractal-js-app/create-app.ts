...
export async function createApp({
  ...
  const template: TemplateType = `${app ? 'app' : 'default'}${empty ? '-empty' : ''}`
  ...
  console.log(`Creating a new Fractal JS app in ${green(root)}.`)
  console.log()
  ...
  if (example) {
    ...
    // Copy `fractal-js-env.d.ts` to any example that is typescript
    const tsconfigPath = join(root, 'tsconfig.json')
    if (existsSync(tsconfigPath)) {
      copyFileSync(
        getTemplateFile({ template, file: 'fractal-js-env.d.ts' }),
        join(root, 'fractal-js-env.d.ts')
      )
    }
