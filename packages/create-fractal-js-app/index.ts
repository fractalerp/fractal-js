...
const program = new Command(packageJson.name)
  .version(
    packageJson.version,
    '-v, --version',
    'Output the current version of create-fractal-js-app.'
  )
  .argument('[directory]')
  ...
  .option(
    ...
    'Reset the preferences saved for create-fractal-js-app.'
  )
  .option(
    '--skip-install',
  ...
  from the official Fractal JS repo or a public GitHub URL. The URL can use
...
async function run(): Promise<void> {
  const conf = new Conf({ projectName: 'create-fractal-js-app' })
  ...
  if (!projectPath) {
    console.log(
      ...
      'For example:\n' +
      `  ${cyan(opts.name())} ${green('my-fractal-js-app')}\n\n` +
    ...
    if (!opts.turbo && !args.includes('--no-turbo')) {
      if (skipPrompt) {
        ...
        const { turbo } = await prompts({
          ...
          type: 'toggle',
          name: 'turbo',
          message: `Would you like to use ${styledTurbo} for ${`fractal-js dev`}?`,
...
async function notifyUpdate(): Promise<void> {
  try {
    if ((await update)?.latest) {
      ...
      const updateMessage = `${global[packageManager]} create-fractal-js-app`
      console.log(
        yellow(bold('A new version of `create-fractal-js-app` is available!')) +
        '\n' +
