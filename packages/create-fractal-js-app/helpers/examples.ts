...
export async function getRepoInfo(
  ...
  if (
    // Support repos whose entire purpose is to be a Fractal JS example, e.g.
    // https://github.com/:username/:my-cool-fractal-js-example-repo-name.
    t === undefined ||
    // Support GitHub URL that ends with a trailing slash, e.g.
    // https://github.com/:username/:my-cool-fractal-js-example-repo-name/
    // In this case "t" will be an empty string while the fractal part "_branch" will be undefined
    (t === '' && _branch === undefined)
  ) {
    try {
...
export function existsInRepo(nameOrUrl: string): Promise<boolean> {
  try {
    ...
    return isUrlOk(
      `https://api.github.com/repos/fractalerp/fractal-js/contents/examples/${encodeURIComponent(
        nameOrUrl
      )}`
    )
  }
...
export async function downloadAndExtractExample(root: string, name: string) {
  ...
  await pipeline(
    await downloadTarStream(
      'https://codeload.github.com/fractalerp/fractal-js/tar.gz/main'
    ),
    x({
      cwd: root,
      strip: 2 + name.split('/').length,
      filter: (p) => p.includes(`fractal-js-main/examples/${name}/`),
    })
  )
}
